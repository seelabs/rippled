...
class Condition:
    ...
    def write_decl(self, f):
        '''
        Condition(Type t, std::uint32_t cost, Slice fingerprint, std::bitset<5> const& subtypes)
        '''
        var_name = self.decl_var_name()
        t = cpp_type(self.type_id())
        digest = self.fingerprint()
        fingerprint_var = '{}ConditionFingerprint'.format(var_name)
        f.write('auto const {var_name}='.format(var_name=fingerprint_var))
        write_cpp_string_literal(f, escape_hex(digest))
        fingerprint_slice = 'makeSlice({var_name})'.format(
            var_name=fingerprint_var)
        cost = str(self.cost())
        subtypes_as_int = 0
        for i in self.fulfillment.subtype_ids():
            subtypes_as_int += 1 << i
        params = ', '.join(
            [t, cost, fingerprint_slice, 'std::bitset<5>{{{}}}'.format(subtypes_as_int)])
    ...
    def write_test(self, f):
        ...
        # swd TBD - check that the decoded condition matches the condition variable and the decoded fingerprint matches the fingerprint
        f.write('}\n')
...
class Fulfillment:
    ...
    def write_test(self, f, is_unique_ptr=False):
        ...
        allfulfillments[-1].write_decl(f, is_unique_ptr=is_unique_ptr)

        var_name = self.decl_var_name()
        f.write('{\n')
        f.write('auto const {var_name}EncodedFulfillment='.format(
            var_name=var_name))
        ...
        f.write('check(')
        if (is_unique_ptr):
            f.write('{}.get(),'.format(var_name))
        else:
            f.write('&{},'.format(var_name))
        f.write('''
        {var_name}Msg,
        {var_name}EncodedFulfillment,
...
class Ed25519(Fulfillment):
    ...
    def write_decl(self, f, is_unique_ptr=False, inc_sub_test=False):
        var_name = self.decl_var_name()
