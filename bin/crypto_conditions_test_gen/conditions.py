...
class Condition:
    ...
    def write_decl(self, f):
        ...
        f.write(
            'std::array<std::uint8_t, 32> const {var_name}={{{{ {array_data} }}}};'.
            format(
                var_name=fingerprint_var, array_data=array_data_hex(digest)))
        cost = str(self.cost())
        subtypes_as_int = 0
        for i in self.fulfillment.subtype_ids():
            subtypes_as_int += 1 << i
        params = ', '.join([
            t, cost, fingerprint_var,
            'std::bitset<5>{{{}}}'.format(subtypes_as_int)
        ])
...
class Fulfillment:
    ...
    def create_from_json(cls, json_init, json_check=None):
        ...
        return prototypes[json_init['type']](
            json_init=json_init, json_check=json_check)
    ...
    def write_test(self, f):
        ...
        var_name = self.decl_var_name()
        f.write('{\n')
        f.write('auto {var_name}EncodedFulfillment='.format(var_name=var_name))
        write_cpp_string_literal(f, escape_hex(encode_it(self.to_asn1())))
        f.write(
            'auto const {var_name}EncodedCondition='.format(var_name=var_name))
...
class Ed25519(Fulfillment):
    ...
    known_keys = [
        load_ed25519_key(k)
        for k in known_signing_keys.ed25519_known_keys_serialized
    ]
...
class RsaSha256(Fulfillment):
    known_keys = [
        load_rsa_key(k) for k in known_signing_keys.rsa_known_keys_serialized
    ]
    ...
    def __init__(self,
        ...
        if pylist is not None and pylist != short_type(self.type_id()):
            raise ValueError('Ill formed pylist spec')
        if signing_key is None:
            signing_key = RsaSha256.known_keys[self.id % len(
                RsaSha256.known_keys)]
...
class Prefix(Fulfillment):
    ...
    def from_json(self, json):
        ...
        self.subfulfillment = Fulfillment.create_from_json(
            json['subfulfillment'])
    ...
    def write_decl(self, f, is_unique_ptr=True, inc_sub_test=False):
        ...
        f.write(
            decl.format(
                var_name=var_name,
                sub_var=self.subfulfillment.decl_var_name()))
...
class Threshold(Fulfillment):
    def __init__(self,
        ...
        if pylist != None:
            ...
            self.subconditions = [
                f.condition()
                for f in Fulfillment.create_from_pylist(pylist[2])
            ]
        self.msg = to_bytes(msg)
        self.set_msg(msg)
    ...
    def cost(self):
        ...
        return 1024 * (len(self.subfulfillments) + len(
            self.subconditions)) + sum(n_highests)
...
class TestWriter:
    ...
    def write_test(self, fulfillment):
        ...
        '''.format(full_test_name=full_test_name, test_name=test_name))
...
def partitioned_test_cases():
    ...
    for tc in ['preim', 'rsa', 'ed']:
        ...
        for i in all_prepre:
            add_to_result(i)
        for a, b, c in zip(all_prepre + all_thresh,
                           itertools.cycle(all_prepre),
                           itertools.cycle(all_thresh)):
            add_to_result(['thresh', [a, b, c], ['preim', 'rsa', 'ed']])
            add_to_result([
                'thresh', [a, 'preim', 'rsa', 'ed'],
                ['preim', 'rsa', 'ed', b, c]
            ])
    return result
...
def save_all_test_cases(file_name_prefix, inc_json=True):
    ...
    for root_condition_name, test_list in test_cases.items():
        ...
        with open(file_name, 'w') as f:
            f.write(
                condition_test_template_prefix.format(
                    RootTestName=root_condition_name))
            tw = TestWriter(f)
            for tc in test_list:
                tw.save_pylist_test_case(tc)
            tw.write_run()
            f.write(
                condition_test_template_suffix.format(
                    RootTestName=root_condition_name))
...
def save_fuzz_corpus(corpus_dir_path):
    ...
    for tc in json_test_cases.test_cases:
        fulfillments.append(
            Fulfillment.create_from_json(json_init=tc['json'], json_check=tc))
