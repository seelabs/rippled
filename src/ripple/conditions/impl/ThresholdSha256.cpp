...
{
    if (subfulfillments_.empty() || subfulfillments_.size() > 65535)
    {
        encoder.ec_ = der::make_error_code(der::Error::thresholdRangeError);
        return;
    }

    std::uint16_t const threshold =
        static_cast<std::uint16_t>(subfulfillments_.size());
    if (!cachedAllConditions_)
    {
        cachedAllConditions_.emplace();
        cachedAllConditions_->reserve(
            subfulfillments_.size() + subconditions_.size());
        for (auto const& f : subfulfillments_)
        {
            cachedAllConditions_->push_back(f->condition(encoder.ec_));
            if (encoder.ec_)
                return;
        }
        for (auto const& c : subconditions_)
            cachedAllConditions_->push_back(c);
    }
    auto conditionsSet =
        der::make_set(*cachedAllConditions_, encoder.traitsCache_);
