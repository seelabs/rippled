    ...
    */
    ...
    for (std::size_t i = 0; i < normPath.size() - 1; ++i)
    {
        ...
        */
        ...
        if (cur->isAccount())
            curIssue.setAccount(cur->getAccountID());
        else if (cur->hasIssuer())
            curIssue.setAccount(cur->getIssuerID());

        if (cur->hasCurrency())
        {
            curIssue.setCurrency(cur->getCurrency());
            if (isXRP(curIssue.currency()))
                curIssue.setAccount(xrpAccount());
        }
    ...
    auto checkStrand = [&]() -> bool {
        ...
        for (auto const& s : result)
        {
            ...
            {
                curIss.setAccount(accts.second);
            }
