            ...
            for (auto const& path : paths)
            {
                try
                {
                    if (exists(path))
                    {
                        ...
                        {
                            JLOG(j_.error()) << "each path must be a directory";
                            return false;
                        }
                    }
...
DatabaseShardImp::checkHistoricalPaths() const
{
    ...
    std::unordered_set<int> filesystemIDs;
    for (auto const& path : historicalPaths_)
    {
        ...
        {
            JLOG(j_.error())
                << "failed to acquire stats for a 'historical_shard_path'";
            return false;
        }

        if (!filesystemIDs.insert(buffer.f_fsid).second)
        {
            JLOG(j_.error())
                << "each 'historical_shard_path' must correspond to a "
                   "distinct device or filesystem";
            return false;
        }
    }
#else
    ...
    for (auto const& path : historicalPaths_)
    {
        auto const availableSpace = boost::filesystem::space(path).available;

        if (uniqueCapacities.find(availableSpace) != uniqueCapacities.end())
            uniqueCapacities[availableSpace].push_back(path.string());
        else
            uniqueCapacities[availableSpace] = {path.string()};
    }
