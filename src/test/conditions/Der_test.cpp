...
class Der_test : public beast::unit_test::suite
{
    ...
    testString()
    {
        ...
        auto testString = [this](
            size_t n, std::vector<char> const& expectedHeader) {
            std::string s;
            s.resize(n);
            ...
            std::fill_n(&expected[headerEnd], n, 'a');
            this->test(s, expected);
        };
        testString(1, {4, 1});
        testString(127, {4, 127});
        testString(128, {4, -127, -128});
        // This disagrees with python, which used the indefinite form
        testString(66000, {4, -125, 1, 1, -48});
    }
