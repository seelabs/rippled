...
class RandomAccountParams
{
    ...
    double const probChangeDefault_ = 0.75;
    // probability that an account redeems into another account
    double const probRedeem_ = 0.5;
...
public:
        ...
        Currency const& currency)
    {
        ...
        // `initialBalance` to make sure the correct balance set correctly.
...
class TheoreticalQuality_test : public beast::unit_test::suite
{
        ...
        boost::optional<Quality> const& expectedQ = {})
    {
        ...
        auto compareClose = [](Quality const& q1, Quality const& q2) {
            ...
            double const tolerence = 0.0000001;
            return relativeDistance(q1, q2) <= tolerence;
        };
...
public:
    ...
    testDirectStep(boost::optional<int> const& reqNumIterations)
    {
        ...
        // For each relavent trust line on the path, there are three things that can vary:
        //  1) input quality
        //  2) output quality
        //  3) debt direction
        ...
        // There are three relavent trust lines: `alice->bob`, `bob->carol`, and
        ...
        // quality values for bob and carol are test almost the same thing.
        ...
        int const numTestIterations = reqNumIterations.value_or(500);

        constexpr std::uint32_t paymentAmount = 1;

        // Class to randommly set account transfer rates, qualities, and other
        // params.
        ...
        {
            auto const iterAsStr = to_string(i);
            ...
            {
                rndAccParams.maybeSetTransferRate(env, accounts[i]);
                // The payment is from:
                ...
                rndAccParams.setupTrustLines(
                    env, accounts[i], accounts[j], currency);
    ...
    testBookStep(boost::optional<int> const& reqNumIterations)
    {
        ...
        // For each relavent trust line, vary input quality, output quality, debt direction.
        // For each account, vary transfer rate.
        // The USD/bob|EUR/carol offer owner is "Oscar"

        // clang-format on

        int const numTestIterations = reqNumIterations.value_or(200);
        ...
        // Sped up tests by creating the environment outside the loop
